...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        await this.sendFile(m.chat, json.nowm, '', '©  Shinichi 真一', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', '©  Shinichi 真一', m)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(url).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), '©  Shinichi 真一', m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(url).then(async res => {
            ...
            await m.reply(wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', '©  Shinichi 真一', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '©  Shinichi 真一', m)
            }
        }).catch(_ => _)
    }
