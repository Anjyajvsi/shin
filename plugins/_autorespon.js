    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Yah.. Shinichi lagi cape (・へ・)' : banned ? 'kamu dibanned' : 'Kenapa Kak,  Kangen ya（ ・∀・）?',
                '',
...
Note: Syarat dan Ketentuan mungkin berlaku`.trim(), wm, 'Payment', '#payment', 'Owner', '#owner', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`⏲️ Rʋи Ƭιмɛ ${uptime} || 🛰️ 𐌼σ∂ɛ: ${global.opts['self'] ? 'Ƥяιѵαтɛ' : setting.groupOnly ? 'Hanya Grup' : 'Ƥʋвℓικ'}`).catch(_ => _)
